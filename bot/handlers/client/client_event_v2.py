...
async def event_v2_start(call: CallbackQuery, state: FSMContext):
    ...
    if state and await state.get_state():
        await state.clear()
    # Тег мероприятия — только в дни ивента (26-27.05.2026 МСК).
    # В остальные дни метить не нужно. Логика та же, что в start_command.
    from bot.handlers.client.client_main import add_user_tag, get_today_event_tag  # type: ignore
    _today_tag = get_today_event_tag()
    if _today_tag:
        asyncio.create_task(add_user_tag(call.from_user.id, _today_tag))
    try:
...
async def _award_ticket(user_id: int, email: str, state: FSMContext):
    ...
    # 0) Если юзеру не положен раффл (тег поставлен анкетой при роли
    #    Рекламодатель/Другое) — пропускаем выдачу билета целиком,
    #    показываем короткое финальное сообщение и выходим.
    skip_raffle = await has_user_tag(user_id, NO_RAFFLE_TAG)
    if skip_raffle:
        await state.clear()
        done_text = (
            '<b>✅ Готово!</b>\n\n'
